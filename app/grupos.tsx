...
const GruposScreen = () => {
  ...
  const [grupoSelecionado, setGrupoSelecionado] = useState<GrupoDisplay | null>(null);
  ...
  const carregarGrupos = async () => {
    ...
    const [{ data: meus, error: erroMeus }, { data: publicos, error: erroPublicos }] =
      await Promise.all([
    ...
    if (erroPublicos) console.error("Erro nos grupos públicos:", erroPublicos.message);

    setMeusGrupos((meus ?? []).map((g) => mapGrupo(g as GrupoSupabase, true)));
    setDescobrirGrupos((publicos ?? []).map((g) => mapGrupo(g as GrupoSupabase, false)));
    setCarregando(false);
  };
